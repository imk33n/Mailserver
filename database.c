...
int db_search(const char *filepath, int start, DBRecord *record) {
	...
	while(1) {
		...
		if(strlen(buffer_key) > 0) {
			...
			/*
			if(strlen(buffer_key) > 0) {	
				if(strcmp(buffer_key, record->key) == 0 && strcmp(buffer_cat, record->cat) == 0) {
					gesetzt = 1;
				}
			*/
...
int main(int argc,  char *argv[]) {
    DBRecord testRec[] = {{"joendhard", "password", "test123"}, {"joendhard", "mailbox", "joendhard.mbox"}, {"joendhard@biffel.xy", "smtp", "joendhard"}, {"pop3", "host", "localhost"}, {"pop3", "port", "6666"}, {"smpt", "host", "localhost"}, {"smtp", "port", "7777"}};
    if(argc > 1){
